"""Train a not_associated vs incidental expert classifier."""
...
def main():
    parser = argparse.ArgumentParser(description="Train not_associated expert")
    ...
    filtered = [r for r in rows if r["label"] in {"not_associated", "incidental"}]
    ...
    payload = {
        ...
        "per_class": {
            "not_associated": {
                "precision": float(report["not_associated"]["precision"]),
                "recall": float(report["not_associated"]["recall"]),
                "f1": float(report["not_associated"]["f1-score"]),
            },
            "incidental": {

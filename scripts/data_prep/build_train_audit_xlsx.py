"""Build relabeling spreadsheet for flagged training examples."""
...
def main():
    with open("data/train_label_audit.json") as f:
        audit = json.load(f)
    with open("data/hfpef_v7_train_relabel3_noleak_large.json") as f:
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Train Relabeling Review"

    headers = [
        "Train Index", "Protein/Biomarker", "Full Sentence", "Current Label",
    ...
    for row_idx, item in enumerate(flagged, 2):
        idx = item["index"]
        full_sentence = train_data[idx]["sentence"]
    ...
    out = Path("data/labeling/train_relabel_audit_164.xlsx")

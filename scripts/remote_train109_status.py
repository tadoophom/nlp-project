"""Inspect the running train109 AWS retrain through Jupyter terminals."""
...
pgrep -af 'run_v9_train109|train_pipeline|python -m src'
python3 - <<'PY'
from pathlib import Path

matches = sorted(Path('logs').glob('v9_retrain_train109_clean_eval_*'))
if matches:
    print('LATEST_RUN_DIR', matches[-1])

model_dirs = sorted(Path('models/hfpef_v9_train109').glob('*'))
if model_dirs:
    print('LATEST_MODEL_DIR', model_dirs[-1])
PY
if [ -f /tmp/run_v9_train109_console.log ]; then
  python3 - <<'PY'
from pathlib import Path

path = Path('/tmp/run_v9_train109_console.log')
...
def main() -> None:
    ...
    try:
        ...
        ws.send(json.dumps(["stdin", STATUS_CMD.replace("\n", "\r")]))
        out: list[str] = []

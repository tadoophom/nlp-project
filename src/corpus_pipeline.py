...
from .pubmedbert_classifier import (
...
def load_entities_file(path: Path) -> Dict[str, List[str]]:
...
def detect_relation_bert(
    text: str,
    *,
    disease_terms: Sequence[str],
    protein_terms: Sequence[str],
    nlp_model,
    bert_classifier: PubMedBERTClassifier,
) -> Dict[str, Optional[str]]:
    """Detect relation using a fine-tuned classifier on co-mention sentences."""
    ...
    hits = extract(text, terms, nlp_model, model_name="scibert")
    if not hits:
        return {
    ...
    for hit in hits:
        ...
        if sent_idx < 0:
            continue
        bucket = by_sentence.setdefault(sent_idx, {"disease": [], "protein": [], "sentence": hit.get("sentence", "")})
        if keyword in disease_set:
            bucket["disease"].append(hit)
        if keyword in protein_set:
            bucket["protein"].append(hit)

    overlapping = [(idx, bucket) for idx, bucket in by_sentence.items() if bucket["disease"] and bucket["protein"]]
    if not overlapping:
        return {
    ...
    for sent_idx, bucket in overlapping:
        sentence = bucket.get("sentence", "")
        if not sentence:
            continue

        result = bert_classifier.classify(sentence)
        if result["confidence"] > best_confidence:
            best_confidence = result["confidence"]
            best_result = {
                "relation": result["polarity"],
                "evidence_sentence": sentence,
                "protein_polarity": result["polarity"],
                "disease_polarity": result["polarity"],
                "protein_confidence": result["confidence"],
...
def build_corpus(
    *,
    ...
    max_protein_terms: int = 0,
) -> List[Dict[str, object]]:
    """Build corpus rows for provided proteins.

    Args:
        use_pubmedbert: If True, use fine-tuned transformer for relation classification.
    ...
    if use_pubmedbert:
        if is_model_available():
            bert_classifier = PubMedBERTClassifier()
            print("Using SciBERT classifier for relation extraction")
        else:
            print("Classifier model not found, falling back to rule-based approach")

    total_proteins = len(protein_entries)
    for idx, entry in enumerate(protein_entries, start=1):
        if idx == 1 or idx % 25 == 0:
            print(f"proteins: {idx}/{total_proteins}, rows: {len(results)}", flush=True)
        if entities is not None:
            protein_terms = [entry.identifier] + entities.get(entry.identifier, [])
        else:
            protein_terms = expand_protein_terms(entry.identifier)
        protein_terms = _dedupe_preserve_order(protein_terms)
        if max_protein_terms and max_protein_terms > 0:
            protein_terms = protein_terms[:max_protein_terms]
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--entities-file", help="Optional pipe-delimited entities file (identifier|syn1|syn2...) to expand protein query terms without UniProt calls")
    parser.add_argument("--max-protein-terms", type=int, default=0, help="Limit number of protein terms used per query (0 means no limit)")
    ...
    parser.add_argument("--use-pubmedbert", action="store_true", help="Use fine-tuned BERT model for relation classification")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    entities = load_entities_file(Path(args.entities_file)) if args.entities_file else None
    ...
    rows = build_corpus(
        ...
        retmax=args.retmax,
        use_pubmedbert=args.use_pubmedbert,
        entities=entities,
        max_protein_terms=args.max_protein_terms,
    )

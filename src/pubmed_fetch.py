...
def _ncbi_get(endpoint, params, retries=5, timeout=20):
    params = dict(params)
    if NCBI_API_KEY:
...
def search_pubmed(keywords: List[str], mesh_terms: List[str], retmax: int = 100):
    ...
    r = _ncbi_get("esearch.fcgi", params)
    r.raise_for_status()
...
def search_pubmed_advanced(
    ...
    if ids is None:
        error = result.get("ERROR") or result.get("errorlist") or data.get("error")
        msg = f"PubMed search failed for query. Error: {error}" if error else "PubMed search failed for query."
        print(msg)
        return [], final_query
